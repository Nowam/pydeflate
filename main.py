...
def compress_file(input_path: Path, output_path: Path | None = None) -> bytes:
...
def decompress_file(input_path: Path, output_path: Path | None = None) -> None:
...
def test_round_trip(input_path: Path) -> None:
    ...
    try:
        ...
        if original_data == decompressed_data:
            ...
            print(
                "✗ Round-trip test FAILED - Data corruption detected", file=sys.stderr
            )
            sys.exit(1)
...
def main():
    ...
    compress_parser.add_argument(
        ...
        "--output",
        type=Path,
        help="Output compressed file (default: input + .deflate)",
    )
    ...
    decompress_parser.add_argument(
        ...
        "--output",
        type=Path,
        help="Output decompressed file (default: remove .deflate extension)",
    )

    # Test command
    test_parser = subparsers.add_parser(
        "test", help="Test compression round-trip on a file"
    )
