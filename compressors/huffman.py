...
class HuffmanCompressor:
    ...
    def _create_huffman_tree(cls, frequencies):
        ...
        queue = []
        res = {letter: 0 for letter in frequencies}
    ...
    def encode(cls, data, alphabet_length=len(ALPHABET), compress_counts=False):
        code_lengths = cls._create_huffman_tree(cls._calculate_empirical_frequency(data))
        code_lengths = [code_lengths[letter] if letter in code_lengths else 0 for letter in range(alphabet_length)]
        codes = {letter: code for code, letter in cls.generate_decode_table(code_lengths).items()}# todo: fix
    ...
    def create_codes(cls, data, alphabet_length):
        code_lengths = cls._create_huffman_tree(cls._calculate_empirical_frequency(data))
        code_lengths = [code_lengths[letter] if letter in code_lengths else 0 for letter
                        in range(alphabet_length)]
        codes = {letter: code for code, letter in
                 cls.generate_decode_table(code_lengths).items()}  # todo: fix
