...
class TestHuffmanCompressor(unittest.TestCase):
    ...
    def test_create_codes_round_trip(self):
        ...
        self.assertLessEqual(
            bit_lengths[65], bit_lengths[67]
        )  # 'A' appears more than 'C'
...
class TestBlockSplitter(unittest.TestCase):
    ...
    def test_initialization(self):
        ...
        self.assertEqual(
            len(self.splitter.observations), self.splitter.NUM_OBSERVATION_TYPES
        )
        self.assertEqual(
            len(self.splitter.new_observations), self.splitter.NUM_OBSERVATION_TYPES
        )
...
def test_compression_ratios_different_data_types(data, description, min_ratio):
    ...
    if min_ratio > 0:
        assert ratio > min_ratio, (
            f"Poor compression for {description}: {ratio:.2f}% < {min_ratio}%"
        )
